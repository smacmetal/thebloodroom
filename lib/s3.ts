...
const S3_SSE = process.env.S3_SSE || ''; // 'AES256' or 'aws:kms'
...
async function withRetry<T>(fn: () => Promise<T>, label: string, tries = 3, delayMs = 150) {
  let lastErr: any;
...
export async function putJsonDeterministic(key: string, data: unknown) {
  await putJson(key, data); // overwrite is fine; key is deterministic by design
}

...
export default function QueenTemplePage() {
  ...
  const deleteMessage = async (id: string) => {
    ...
    if (!ok) return;
    const res = await fetch(`/api/queen/messages?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
    if (!res.ok) {
      const txt = await res.text().catch(() => '');
  ...
  return (
    <div className="min-h-screen bg-black text-pink-200">
      <div className="max-w-5xl mx-auto px-4 py-6">
        <h1 className="text-2xl font-bold mb-4">Queen Messages</h1>
        ...
        <div className="rounded-xl border border-pink-600 p-4 mb-8">
          <label className="block text-sm mb-2">Send to Queen</label>
          <input
            value={title}

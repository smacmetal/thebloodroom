...
// GET — return all King messages with signed file URLs (if any)
export async function GET() {
  try {
    const keys = await listJsonUnder('messages/king/');
    ...
    const messages = await Promise.all(
      keys.map(async (key) => {
        try {
          ...
          if (rec?.filename) {
            const s3Key = `messages/king/files/${rec.filename}`;
    ...
    console.error('GET /king/messages failed:', err);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (ct.includes('application/json')) {
      ...
      await putJson(`messages/king/${timestamp}.json`, record);
    ...
    if (ct.includes('multipart/form-data')) {
      ...
      if (incoming) {
        ...
        await uploadFileToS3(`messages/king/files/${unique}`, fileBuffer, incoming.headers['content-type']);
        filename = unique;
      }
    ...
    console.error('POST /king/messages failed:', err);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const jsonKey = `messages/king/${ts}.json`;
    ...
    if (filename) {
      const fileKey = `messages/king/files/${filename}`;
      await s3.send(new DeleteObjectCommand({ Bucket: BUCKET, Key: fileKey }));
    }

    return NextResponse.json({ success: true });
  } catch (err) {
    console.error('DELETE /king/messages failed:', err);

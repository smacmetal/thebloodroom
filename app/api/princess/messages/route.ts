...
const MESSAGES_DIR = path.join(ROOT, "data", "princess", "messages");
...
export async function GET() {
  try {
    ...
    console.error("Error reading Princess messages:", err);
...
export async function POST(req: Request) {
  try {
    ...
    const now = Date.now();
    const id = `${now}-princess`;
    const payload = {
      id,
      author: data.author ?? "King",
      title: typeof data.title === "string" ? data.title : "",
      body: typeof data.body === "string" ? data.body : "",
      recipients: Array.isArray(data.recipients) ? data.recipients : ["Princess"],
    ...
    console.error("Error writing Princess message:", err);
...
export async function DELETE(req: Request) {
  try {
    ...
    console.error("Error deleting Princess message:", err);
